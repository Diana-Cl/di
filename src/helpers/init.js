...
export function initializeParams(request, env) {
  ...
  globalThis.proxyIP = proxyIPs
    ...
    : atob("bmltYS5uc2NsLmly"); // decode with https://www.base64decode.org/
  ...
  if (pathName !== "/secrets") {
    if (!userID || !trojanPassword)
      throw new Error(
        `Please set UUID and Trojan password first. Please visit <a href="https://${hostName}/secrets" target="_blank">here</a> to generate them.`,
        { cause: "init" },
      );
