...
export async function getSingBoxWarpConfig(request, env, client) {
  ...
  const dnsObject = buildSingBoxDNS(
    proxySettings,
    undefined,
    true,
  ...
  selector.outbounds = [`🔴 Warp${proIndicator}- Best Ping`, `🔴 Warp${proIndicator}- Best Ping`];
  ...
  WoWUrlTest.tag = `🔴 Warp${proIndicator}- Best Ping`;
...
export async function getSingBoxCustomConfig(request, env, isFragment) {
  ...
  let proxyIndex = 1;
  const protocols = [...(vlessConfigs ? ['VLESS'] : []), ...(trojanConfigs ? ['Trojan'] : [])];
