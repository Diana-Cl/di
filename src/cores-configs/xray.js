...
async function buildXrayWorkerLessConfig(proxySettings) {
  ...
  const fakeOutbound = buildXrayVLESSOutbound(
    "fake-outbound",
    "google.com",
    "443",
    globalThis.userID,
  ...
  for (const protocol of protocols) {
    ...
    for (const port of totalPorts) {
      for (const addr of totalAddresses) {
        ...
        const customConfig = buildXrayConfig(
          proxySettings,
          remark,
          isFragment,
          false,
...
export async function getXrayWarpConfigs(request, env, client) {
  ...
  for (const [index, endpoint] of warpEndpoints.split(",").entries()) {
    ...
    const warpConfig = buildXrayConfig(
      proxySettings,
    ...
    const WoWConfig = buildXrayConfig(
      proxySettings,
  ...
  const xrayWarpBestPing = buildXrayConfig(
    proxySettings,
  ...
  const xrayWoWBestPing = buildXrayConfig(
    proxySettings,
