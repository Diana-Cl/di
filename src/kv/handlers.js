...
export async function updateDataset(request, env) {
  ...
  if (isServerDomain) {
    try {
      ...
      resolvedRemoteDNS = {
        ...
        staticIPs: enableIPv6
          ? [...resolvedDomain.ipv4, ...resolvedDomain.ipv6]
          : resolvedDomain.ipv4,
      };
    } catch (error) {
      console.log(error);
  ...
  const proxySettings = {
    ...
    vlessTrojanFakeDNS:
      validateField('vlessTrojanFakeDNS') ?? currentSettings?.vlessTrojanFakeDNS ?? false,
    ...
    bestVLESSTrojanInterval:
      validateField('bestVLESSTrojanInterval') ?? currentSettings?.bestVLESSTrojanInterval ?? '30',
